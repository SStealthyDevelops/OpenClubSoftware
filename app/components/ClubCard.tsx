...
function ClubCardSkeleton() {
    return (
        <div className="">
            <div className="flex flex-col m-4 min-h-64 h-fit card relative rounded-xl hover:scale-105 duration-200 border-4 border-gray-400">
                <div
                    className="relative rounded-[0.45rem] flex flex-row bg-cover bg-center h-24 border-b-0 bg-gray-500"
                >
                    {/* GRADIENT OVERLAY */}
                    <div
                        className="absolute inset-0 bg-gradient-to-b from-transparent to-white opacity-20 rounded-t-xl"/>
                </div>
                <div className="flex flex-row">
                    ...
                    <div className="flex flex-1 justify-end">
                        <div
                            className="text-xl title h-8 w-32 whitespace-normal text-right mr-5 mt-3 break-words bg-gray-500 rounded-xl"/>
                    </div>
                </div>
...
export const ClubCard: React.FC<ClubInfo> = ({id, name, slug, logo, banner, description, accent}) => {
